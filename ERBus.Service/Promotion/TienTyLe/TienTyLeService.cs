...
{
    public interface ITienTyLeService : IDataInfoService<KHUYENMAI>
    {
        string BuildCode();
        string SaveCode();
        KHUYENMAI InsertKhuyenMai(TienTyLeViewModel.Dto instance);
        KHUYENMAI UpdateKhuyenMai(TienTyLeViewModel.Dto instance);
        bool DeleteKhuyenMai(string ID);
        PagedObj<KHUYENMAI> QueryPageTienTyLe(string stringConnect, PagedObj<KHUYENMAI> page, string strKey, string maDonVi);
    }
    public class TienTyLeService : DataInfoServiceBase<KHUYENMAI>, ITienTyLeService
    {
        public TienTyLeService(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }
        ...
        {
            var unitCode = GetCurrentUnitCode();
            var type = TypeBuildCode.TIENTYLE.ToString();
            var result = "";
        ...
        public PagedObj<KHUYENMAI> QueryPageTienTyLe(string stringConnect, PagedObj<KHUYENMAI> page, string strKey, string maDonVi)
        {
            var TotalItem = 0;
                ...
                using (OracleConnection connection = new OracleConnection(stringConnect))
                {
                    try
                    {
                        ...
                        if (connection.State == ConnectionState.Open)
                        {
                            ...
                            command.Parameters.Add(@"P_LOAI_KHUYENMAI", OracleDbType.NVarchar2, 20).Value = TypeBuildCode.TIENTYLE.ToString();
        ...
        public KHUYENMAI InsertKhuyenMai(TienTyLeViewModel.Dto instance)
        {
            var dataKhuyenMai = Mapper.Map<TienTyLeViewModel.Dto, KHUYENMAI>(instance);
            dataKhuyenMai.ID = Guid.NewGuid().ToString();
            dataKhuyenMai.LOAI_KHUYENMAI = TypeBuildCode.TIENTYLE.ToString();
            ...
            var dataDetails = Mapper.Map<List<TienTyLeViewModel.DtoDetails>, List<KHUYENMAI_CHITIET>>(instance.DataDetails);
            dataDetails.ForEach(x =>
            {
                x.ID = Guid.NewGuid().ToString();
                x.MA_KHUYENMAI = result.MA_KHUYENMAI;
            });
            result = Insert(result);
            UnitOfWork.Repository<KHUYENMAI_CHITIET>().InsertRange(dataDetails);
            return result;
        }
        ...
        public KHUYENMAI UpdateKhuyenMai(TienTyLeViewModel.Dto instance)
        {
            ...
            {
                x.ID = Guid.NewGuid().ToString();
                x.MA_KHUYENMAI = dataKhuyenMai.MA_KHUYENMAI;
            });
            UnitOfWork.Repository<KHUYENMAI_CHITIET>().InsertRange(dataDetails);
