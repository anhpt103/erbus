...
namespace ERBus.Service.Promotion.GiamGiaNhomHang
{
    public interface IGiamGiaNhomHangService : IDataInfoService<KHUYENMAI>
    {
        string BuildCode();
        string SaveCode();
        KHUYENMAI InsertKhuyenMai(GiamGiaNhomHangViewModel.Dto instance);
        KHUYENMAI UpdateKhuyenMai(GiamGiaNhomHangViewModel.Dto instance);
        bool DeleteKhuyenMai(string ID);
        PagedObj<KHUYENMAI> QueryPageGiamGiaNhomHang(string stringConnect, PagedObj<KHUYENMAI> page, string strKey, string maDonVi);
    }
    public class GiamGiaNhomHangService : DataInfoServiceBase<KHUYENMAI>, IGiamGiaNhomHangService
    {
        public GiamGiaNhomHangService(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }
        ...
        {
            var unitCode = GetCurrentUnitCode();
            var type = TypeBuildCode.GGNHOMHANG.ToString();
            var result = "";
        ...
        public PagedObj<KHUYENMAI> QueryPageGiamGiaNhomHang(string stringConnect, PagedObj<KHUYENMAI> page, string strKey, string maDonVi)
        {
            var TotalItem = 0;
                ...
                using (OracleConnection connection = new OracleConnection(stringConnect))
                {
                    try
                    {
                        ...
                        if (connection.State == ConnectionState.Open)
                        {
                            ...
                            command.Parameters.Add(@"P_LOAI_KHUYENMAI", OracleDbType.NVarchar2, 20).Value = TypeBuildCode.GGNHOMHANG.ToString();
        ...
        public KHUYENMAI InsertKhuyenMai(GiamGiaNhomHangViewModel.Dto instance)
        {
            var dataKhuyenMai = Mapper.Map<GiamGiaNhomHangViewModel.Dto, KHUYENMAI>(instance);
            dataKhuyenMai.ID = Guid.NewGuid().ToString();
            dataKhuyenMai.LOAI_KHUYENMAI = TypeBuildCode.GGNHOMHANG.ToString();
            ...
            if (instance.DataDetails.Count > 0)
            {
                foreach (var detail in instance.DataDetails)
                {
                    ...
                    khuyenMaiChiTiet.MAHANG = detail.MANHOM;
        ...
        public KHUYENMAI UpdateKhuyenMai(GiamGiaNhomHangViewModel.Dto instance)
        {
            var dataKhuyenMai = Mapper.Map<GiamGiaNhomHangViewModel.Dto, KHUYENMAI>(instance);
