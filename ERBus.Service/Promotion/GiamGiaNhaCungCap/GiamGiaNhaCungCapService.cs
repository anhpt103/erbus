...
namespace ERBus.Service.Promotion.GiamGiaNhaCungCap
{
    public interface IGiamGiaNhaCungCapService : IDataInfoService<KHUYENMAI>
    {
        string BuildCode();
        string SaveCode();
        KHUYENMAI InsertKhuyenMai(GiamGiaNhaCungCapViewModel.Dto instance);
        KHUYENMAI UpdateKhuyenMai(GiamGiaNhaCungCapViewModel.Dto instance);
        bool DeleteKhuyenMai(string ID);
        PagedObj<KHUYENMAI> QueryPageGiamGiaNhaCungCap(string stringConnect, PagedObj<KHUYENMAI> page, string strKey, string maDonVi);
    }
    public class GiamGiaNhaCungCapService : DataInfoServiceBase<KHUYENMAI>, IGiamGiaNhaCungCapService
    {
        public GiamGiaNhaCungCapService(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }
        ...
        {
            var unitCode = GetCurrentUnitCode();
            var type = TypeBuildCode.GGNHACUNGCAP.ToString();
            var result = "";
        ...
        public KHUYENMAI InsertKhuyenMai(GiamGiaNhaCungCapViewModel.Dto instance)
        {
            var dataKhuyenMai = Mapper.Map<GiamGiaNhaCungCapViewModel.Dto, KHUYENMAI>(instance);
            dataKhuyenMai.ID = Guid.NewGuid().ToString();
            dataKhuyenMai.LOAI_KHUYENMAI = TypeBuildCode.GGNHACUNGCAP.ToString();
            ...
            if (instance.DataDetails.Count > 0)
            {
                foreach (var detail in instance.DataDetails)
                {
                    ...
                    khuyenMaiChiTiet.MAHANG = detail.MANHACUNGCAP;
        ...
        public KHUYENMAI UpdateKhuyenMai(GiamGiaNhaCungCapViewModel.Dto instance)
        {
            var dataKhuyenMai = Mapper.Map<GiamGiaNhaCungCapViewModel.Dto, KHUYENMAI>(instance);
