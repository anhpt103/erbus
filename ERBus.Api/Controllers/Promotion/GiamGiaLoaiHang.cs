...
{
    [RoutePrefix("api/Promotion/GiamGiaLoaiHang")]
    [Route("{id?}")]
    [Authorize]
    public class GiamGiaLoaiHangController : ApiController
    {
        private IGiamGiaLoaiHangService _service;
        public GiamGiaLoaiHangController(IGiamGiaLoaiHangService service)
        {
            _service = service;
        }
        ...
        [Route("PostQuery")]
        [HttpPost]
        [CustomAuthorize(Method = "XEM", State = "GiamGiaLoaiHang")]
        public IHttpActionResult PostQuery(JObject jsonData)
        {
            ...
            var filtered = ((JObject)postData.filtered).ToObject<FilterObj<GiamGiaLoaiHangViewModel.Search>>();
            var paged = ((JObject)postData.paged).ToObject<PagedObj<KHUYENMAI>>();
            var unitCode = _service.GetCurrentUnitCode();
            try
            {
                PagedObj<KHUYENMAI> tempData = _service.QueryPageGiamGiaLoaiHang(_service.GetConnectionString(), paged, filtered.Summary, unitCode);
        ...
        [CustomAuthorize(Method = "THEM", State = "GiamGiaLoaiHang")]
        public async Task<IHttpActionResult> Post(GiamGiaLoaiHangViewModel.Dto instance)
        {
        ...
        {
            var result = new TransferObj<GiamGiaLoaiHangViewModel.Dto>();
            GiamGiaLoaiHangViewModel.Dto dto = new GiamGiaLoaiHangViewModel.Dto();
            ...
            else
            {
                ...
                if (chungTu != null)
                {
                    dto = Mapper.Map<KHUYENMAI, GiamGiaLoaiHangViewModel.Dto>(chungTu);
                    List<GiamGiaLoaiHangViewModel.DtoDetails> listDetails = new List<GiamGiaLoaiHangViewModel.DtoDetails>();
                    ...
                    if(chungTuChiTiet.Count > 0)
                    {
                        ...
                        {
                            GiamGiaLoaiHangViewModel.DtoDetails details = new GiamGiaLoaiHangViewModel.DtoDetails();
                            details.ID = row.ID;
                            details.MALOAI = row.MAHANG;
                            details.SOLUONG = row.SOLUONG;
                            var loaiHang = _service.UnitOfWork.Repository<LOAIHANG>().DbSet.FirstOrDefault(x => x.MALOAI.Equals(row.MAHANG));
                            details.TENLOAI = loaiHang != null ? loaiHang.TENLOAI : "";
        ...
        [CustomAuthorize(Method = "SUA", State = "GiamGiaLoaiHang")]
        public async Task<IHttpActionResult> Put(string id, GiamGiaLoaiHangViewModel.Dto instance)
        {
        ...
        [ResponseType(typeof(KHUYENMAI))]
        [CustomAuthorize(Method = "XOA", State = "GiamGiaLoaiHang")]
        ...
        [Route("PostApproval")]
        [CustomAuthorize(Method = "DUYET", State = "GiamGiaLoaiHang")]
        public async Task<IHttpActionResult> PostApproval(GiamGiaLoaiHangViewModel.ParamApproval instance)
        {
        ...
        [CustomAuthorize(Method = "DUYET", State = "GiamGiaLoaiHang")]
        public async Task<IHttpActionResult> PostUnApproval(GiamGiaLoaiHangViewModel.ParamApproval instance)
        {
            var result = new TransferObj();
            string unitCode = _service.GetCurrentUnitCode();
            if (!string.IsNullOrEmpty(instance.ID))
