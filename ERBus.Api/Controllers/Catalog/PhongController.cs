...
namespace ERBus.Api.Controllers.Catalog
{
    ...
    public class PhongController : ApiController
    {
        ...
        public IHttpActionResult GetAllData()
        {
            ...
            result.Data = _service.Repository.DbSet.Where(x => x.TRANGTHAI == (int)TypeState.USED && x.UNITCODE.Equals(unitCode)).OrderBy(x => x.MAPHONG).Select(x => new ChoiceObject { VALUE = x.MAPHONG, TEXT = x.MAPHONG + " | " + x.TENPHONG, DESCRIPTION = x.TENPHONG, PARENT = x.TANG != null ? x.TANG.ToString() : "", ID = x.ID }).ToList();
