...
BEGIN
  ...
  BEGIN 
       SELECT CASE WHEN length(convert(P_TUKHOA, 'utf8', 'us7ascii')) > length(P_TUKHOA) THEN 'X' ELSE NULL END INTO IS_CONTAIN_UNITCODE FROM DUAL;
...
BEGIN
  ...
  BEGIN 
       SELECT CASE WHEN length(convert(P_KEYSEARCH, 'utf8', 'us7ascii')) > length(P_KEYSEARCH) THEN 'X' ELSE NULL END INTO IS_CONTAIN_UNITCODE FROM DUAL;
          EXCEPTION WHEN NO_DATA_FOUND
          THEN IS_CONTAIN_UNITCODE := '';
  END;
