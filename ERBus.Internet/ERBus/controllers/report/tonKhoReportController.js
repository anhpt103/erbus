define(['ui-bootstrap', 'controllers/catalog/khoHangController', 'controllers/catalog/loaiHangController', 'controllers/catalog/nhomHangController', 'controllers/catalog/nhaCungCapController', 'controllers/catalog/matHangController', 'controllers/authorize/cuaHangController', 'controllers/authorize/authController', 'controllers/authorize/kyKeToanController'], function () {
    'use strict';
    var app = angular.module('tonKhoModule', ['ui.bootstrap', 'khoHangModule', 'loaiHangModule', 'nhomHangModule', 'nhaCungCapModule', 'matHangModule', 'cuaHangModule', 'kyKeToanModule']);
    app.factory('tonKhoService', ['$http', 'configService', function ($http, configService) {
        var serviceUrl = configService.rootUrlWebApi + '/Report/TonKho';
        var selectedData = [];
        var result = {
    ...
    /* controller list */
    app.controller('BaoCaoTonKho_Ctrl', ['$scope', '$http', 'configService', 'tonKhoService', 'tempDataService', '$filter', '$uibModal', '$log', 'securityService', 'khoHangService', 'loaiHangService', 'nhomHangService', 'nhaCungCapService', 'matHangService', 'cuaHangService', 'userService', 'kyKeToanService',
        function ($scope, $http, configService, service, tempDataService, $filter, $uibModal, $log, securityService, khoHangService, loaiHangService, nhomHangService, nhaCungCapService, matHangService, cuaHangService, userService, kyKeToanService) {
            ...
            $scope.title = function () { return 'Báo cáo tồn kho' };
            $scope.target = {
                MA_BAOCAO: '',
            ...
            //end
            $scope.target.MA_BAOCAO = "BAOCAO_TONKHO";
            ...
            function loadAccessList() {
                ...
                securityService.getAccessList('BaoCaoTonKho', userName, unitCodeParam).then(function (successRes) {
            ...
            $scope.reportDetail = function () {
                if (!$scope.target.TEN_CUAHANG || $scope.target.TEN_CUAHANG == "") {
                    ...
                    var modalInstance = $uibModal.open({
                        ...
                        keyboard: false,
                        templateUrl: configService.buildUrl('report/Template', 'reports.template'),
                        controller: 'TonKhoReportDetail_Ctrl',
                        resolve: {
                            objParam: function () {
                                return $scope.target;
                            }
    ...
    app.controller('TonKhoReport_Ctrl', ['$scope', '$http', 'configService', 'objParam','$uibModalInstance','$filter','kyKeToanService',
        function ($scope, $http, configService, objParam, $uibModalInstance, $filter, kyKeToanService) {
            ...
            $scope.report = {
                name: "ERBus.Api.Reports.TonKho.TonKho_TongHop,ERBus.Api",
    ...
    app.controller('TonKhoReportDetail_Ctrl', ['$scope', '$http', 'configService', 'objParam', '$uibModalInstance', '$filter','kyKeToanService',
        function ($scope, $http, configService, objParam, $uibModalInstance, $filter, kyKeToanService) {
            $scope.config = angular.copy(configService);
            function convertArrayToString(arr) {
                var result = '';
            ...
            $scope.report = {
                name: "ERBus.Api.Reports.TonKho.TonKho_ChiTiet,ERBus.Api",
