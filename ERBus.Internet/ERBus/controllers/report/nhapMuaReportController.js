define(['ui-bootstrap', 'controllers/catalog/khoHangController', 'controllers/catalog/loaiHangController', 'controllers/catalog/nhomHangController', 'controllers/catalog/nhaCungCapController', 'controllers/catalog/matHangController', 'controllers/authorize/cuaHangController', 'controllers/authorize/authController', 'controllers/authorize/kyKeToanController'], function () {
    'use strict';
    var app = angular.module('nhapMuaModule', ['ui.bootstrap', 'khoHangModule', 'loaiHangModule', 'nhomHangModule', 'nhaCungCapModule', 'matHangModule', 'cuaHangModule', 'kyKeToanModule']);
    app.factory('nhapMuaService', ['$http', 'configService', function ($http, configService) {
        var serviceUrl = configService.rootUrlWebApi + '/Report/NhapMua';
        var selectedData = [];
        var result = {
    ...
    /* controller list */
    app.controller('BaoCaoNhapMua_Ctrl', ['$scope', '$http', 'configService', 'nhapMuaService', 'tempDataService', '$filter', '$uibModal', '$log', 'securityService', 'khoHangService', 'loaiHangService', 'nhomHangService', 'nhaCungCapService', 'matHangService', 'cuaHangService', 'userService', 'kyKeToanService',
        function ($scope, $http, configService, service, tempDataService, $filter, $uibModal, $log, securityService, khoHangService, loaiHangService, nhomHangService, nhaCungCapService, matHangService, cuaHangService, userService, kyKeToanService) {
            ...
            $scope.title = function () { return 'Báo cáo nhập mua' };
            $scope.target = {
                MA_BAOCAO: '',
            ...
            //end
            $scope.target.MA_BAOCAO = "BAOCAO_NHAPMUA";
            ...
            function loadAccessList() {
                ...
                securityService.getAccessList('BaoCaoNhapMua', userName, unitCodeParam).then(function (successRes) {
            ...
            $scope.report = function () {
                if (!$scope.target.TEN_CUAHANG || $scope.target.TEN_CUAHANG == "") {
                    ...
                    var modalInstance = $uibModal.open({
                        ...
                        controller: 'NhapMuaReportReport_Ctrl',
                        resolve: {
                            objParam: function () {
                                return $scope.target;
                            }
            ...
            $scope.reportDetail = function () {
                if (!$scope.target.TEN_CUAHANG || $scope.target.TEN_CUAHANG == "") {
                    ...
                    var modalInstance = $uibModal.open({
                        ...
                        keyboard: false,
                        templateUrl: configService.buildUrl('report/Template', 'reports.template'),
                        controller: 'NhapMuaReportReportDetail_Ctrl',
                        resolve: {
                            objParam: function () {
                                return $scope.target;
                            }
    ...
    app.controller('NhapMuaReportReport_Ctrl', ['$scope', '$http', 'configService', 'objParam','$uibModalInstance','$filter','kyKeToanService',
        function ($scope, $http, configService, objParam, $uibModalInstance, $filter, kyKeToanService) {
            ...
            paramsReport.LOAI_CHUNGTU = "NMUA";
            ...
            $scope.report = {
                name: "ERBus.Api.Reports.NhapMua.NHAPMUA_TONGHOP,ERBus.Api",
                title: "Báo cáo nhập mua tổng hợp",
                params: paramsReport
            };
            $scope.cancel = function () {
    ...
    app.controller('NhapMuaReportReportDetail_Ctrl', ['$scope', '$http', 'configService', 'objParam', '$uibModalInstance', '$filter','kyKeToanService',
        function ($scope, $http, configService, objParam, $uibModalInstance, $filter, kyKeToanService) {
            $scope.config = angular.copy(configService);
            function convertArrayToString(arr) {
                var result = '';
            ...
            $scope.report = {
                name: "ERBus.Api.Reports.NhapMua.NHAPMUA_CHITIET,ERBus.Api",
                title: "Báo cáo nhập mua chi tiết",
                params: paramsReport
            };
            $scope.cancel = function () {
                $uibModalInstance.close(paramsReport);
