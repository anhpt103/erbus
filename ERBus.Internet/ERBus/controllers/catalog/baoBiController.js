define(['ui-bootstrap'], function () {
    'use strict';
    var app = angular.module('baoBiModule', ['ui.bootstrap']);
    app.factory('baoBiService', ['$http', 'configService', function ($http, configService) {
        var serviceUrl = configService.rootUrlWebApi + '/Catalog/BaoBi';
        var selectedData = [];
        var result = {
    ...
    app.controller('BaoBi_Ctrl', ['$scope', '$http', 'configService', 'baoBiService', 'tempDataService', '$filter', '$uibModal', '$log', 'securityService','userService',
        function ($scope, $http, configService, service, tempDataService, $filter, $uibModal, $log, securityService, userService) {
            ...
            $scope.title = function () { return 'Bao bì hàng hóa' };
            $scope.data = [];
            $scope.setPage = function (pageNo) {
                $scope.paged.CurrentPage = pageNo;
                filterData();
            };
            $scope.sortType = 'MABAOBI';
            ...
            $scope.create = function () {
                var modalInstance = $uibModal.open({
                    ...
                    size: 'lg',
                    templateUrl: configService.buildUrl('catalog/BaoBi', 'create'),
                    controller: 'baoBiCreate_Ctrl',
                    resolve: {}
                });
            ...
            $scope.detail = function (target) {
                var modalInstance = $uibModal.open({
                    ...
                    animation: true,
                    size: 'lg',
                    templateUrl: configService.buildUrl('catalog/BaoBi', 'detail'),
                    controller: 'baoBiDetail_Ctrl',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
            ...
            $scope.edit = function (target) {
                var modalInstance = $uibModal.open({
                    ...
                    animation: true,
                    size: 'lg',
                    templateUrl: configService.buildUrl('catalog/BaoBi', 'edit'),
                    controller: 'baoBiEdit_Ctrl',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
            ...
            $scope.delete = function (event, target) {
                var modalInstance = $uibModal.open({
                    ...
                    templateUrl: configService.buildUrl('catalog/BaoBi', 'delete'),
                    controller: 'baoBiDelete_Ctrl',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
    ...
    app.controller('baoBiCreate_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'baoBiService', 'tempDataService', '$filter', '$uibModal', '$log',
        function ($scope, $uibModalInstance, $http, configService, service, tempDataService, $filter, $uibModal, $log) {
            $scope.config = angular.copy(configService);
            $scope.tempData = tempDataService.tempData;
            $scope.title = function () { return 'Thêm bao bì sản phẩm'; };
            $scope.target = {};
            //Tạo mới mã loại
            service.buildNewCode().then(function (successRes) {
                if (successRes && successRes.status == 200 && successRes.data) {
                    $scope.target.MABAOBI = successRes.data;
                }
            });
            //end
            $scope.save = function () {
                if (!$scope.target.MABAOBI || !$scope.target.TENBAOBI) {
    ...
    app.controller('baoBiDetail_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'baoBiService', 'targetData', 'tempDataService', '$filter', '$uibModal', '$log',
        function ($scope, $uibModalInstance, $http, configService, service, targetData, tempDataService, $filter, $uibModal, $log) {
            $scope.config = angular.copy(configService);
            $scope.tempData = tempDataService.tempData;
            $scope.target = {};
            $scope.target = angular.copy(targetData);
            $scope.title = function () { return 'Thông tin bao bì sản phẩm [' + targetData.MABAOBI + ']'; };
            $scope.cancel = function () {
                $uibModalInstance.close();
            };
        }]);
    app.controller('baoBiEdit_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'baoBiService', 'targetData', 'tempDataService', '$filter', '$uibModal', '$log',
        function ($scope, $uibModalInstance, $http, configService, service, targetData, tempDataService, $filter, $uibModal, $log) {
            $scope.config = angular.copy(configService);
            $scope.tempData = tempDataService.tempData;
            $scope.target = {};
            $scope.target = angular.copy(targetData);
            $scope.title = function () { return 'Chỉnh sửa bao bì sản phẩm'; };
            $scope.save = function () {
                if (!$scope.target.MABAOBI || !$scope.target.TENBAOBI) {
                    Lobibox.notify('warning', {
                        title: 'Thiếu thông tin',
    ...
    app.controller('baoBiDelete_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'baoBiService', 'targetData', 'tempDataService', '$filter', '$uibModal', '$log',
       function ($scope, $uibModalInstance, $http, configService, service, targetData, tempDataService, $filter, $uibModal, $log) {
           $scope.config = angular.copy(configService);
           $scope.tempData = tempDataService.tempData;
           $scope.target = {};
           $scope.target = angular.copy(targetData);;
           $scope.title = function () { return 'Xóa bao bì sản phẩm [' + targetData.MABAOBI + ']'; };
