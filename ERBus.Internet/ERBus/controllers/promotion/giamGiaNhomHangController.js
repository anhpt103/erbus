define(['ui-bootstrap', 'controllers/catalog/khoHangController', 'controllers/catalog/nhomHangController'], function () {
    'use strict';
    var app = angular.module('giamGiaNhomHangModule', ['ui.bootstrap', 'khoHangModule', 'nhomHangModule']);
    app.factory('giamGiaNhomHangService', ['$http', 'configService', function ($http, configService) {
        var serviceUrl = configService.rootUrlWebApi + '/Promotion/GiamGiaNhomHang';
        var selectedData = [];
        var result = {
    ...
    /* controller list */
    app.controller('GiamGiaNhomHang_Ctrl', ['$scope', '$http', 'configService', 'giamGiaNhomHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'securityService', 'khoHangService','nhomHangService','userService',
        function ($scope, $http, configService, service, tempDataService, $filter, $uibModal, $log, securityService, khoHangService, nhomHangService, userService) {
            $scope.config = angular.copy(configService);
            $scope.paged = angular.copy(configService.pageDefault);
            $scope.filtered = angular.copy(configService.filterDefault);
            $scope.tempData = tempDataService.tempData;
            $scope.title = function () { return 'Khuyến mãi giảm giá theo nhóm hàng' };
            $scope.data = [];
            ...
            function loadAccessList() {
                ...
                securityService.getAccessList('GiamGiaNhomHang', userName, unitCodeParam).then(function (successRes) {
            ...
            $scope.create = function () {
                var modalInstance = $uibModal.open({
                    ...
                    templateUrl: configService.buildUrl('promotion/GiamGiaNhomHang', 'create'),
                    controller: 'giamGiaNhomHangCreate_Ctrl',
                    resolve: {}
                });
            ...
            $scope.detail = function (target) {
                var modalInstance = $uibModal.open({
                    ...
                    animation: true,
                    windowClass: 'promotion-window',
                    templateUrl: configService.buildUrl('promotion/GiamGiaNhomHang', 'detail'),
                    controller: 'giamGiaNhomHangDetail_Ctrl',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
            ...
            $scope.edit = function (target) {
                var modalInstance = $uibModal.open({
                    ...
                    animation: true,
                    windowClass: 'promotion-window',
                    templateUrl: configService.buildUrl('promotion/GiamGiaNhomHang', 'edit'),
                    controller: 'giamGiaNhomHangEdit_Ctrl',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
            ...
            $scope.delete = function (event, target) {
                var modalInstance = $uibModal.open({
                    ...
                    templateUrl: configService.buildUrl('promotion/GiamGiaNhomHang', 'delete'),
                    controller: 'giamGiaNhomHangDelete_Ctrl',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
    ...
    app.controller('giamGiaNhomHangCreate_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'giamGiaNhomHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'khoHangService','userService','nhomHangService',
    function ($scope, $uibModalInstance, $http, configService, service, tempDataService, $filter, $uibModal, $log, khoHangService, userService, nhomHangService) {
        $scope.config = angular.copy(configService);
        $scope.tempData = tempDataService.tempData;
        $scope.paged = angular.copy(configService.pageDefault);
        var currentUser = userService.GetCurrentUser();
        var unitCode = currentUser.unitCode;
        $scope.title = function () { return 'Thêm chương trình khuyến mãi giảm giá theo nhóm hàng'; };
        $scope.target = {
            TUNGAY: new Date(),
            DENNGAY: new Date(),
            TRANGTHAI: 0,
        ...
        function errorFocusMaHang() {
            focus('_maNhomAddItem');
            document.getElementById('_maNhomAddItem').focus();
            document.getElementById('_maNhomAddItem').select();
        };
        ...
        //end 
        $scope.selectMaNhom = function (maLoai) {
            if (maLoai) {
                var tempCache = $filter('filter')($scope.nhomHang, { VALUE: maLoai }, true);
                if (tempCache && tempCache.length === 1) {
                    $scope.addItem.TENNHOM = tempCache[0].DESCRIPTION;
                }
            }
        ...
        $scope.addRow = function () {
            if ($scope.addItem && $scope.addItem.MANHOM !== '') {
                var exist = $filter('filter')($scope.target.DataDetails, { MANHOM: $scope.addItem.MANHOM }, true);
                if (exist && exist.length == 1) {
                    ...
                    Lobibox.notify('success', {
                        title: 'Thông báo',
                        width: 400,
                        msg: "Đã tồn tại loại hàng " + exist[0].MANHOM + ".",
                        delay: 1500
                    });
                }
                else {
        ...
        $scope.save = function () {
            if (!$scope.target.MA_KHUYENMAI || !$scope.target.TUNGAY || !$scope.target.DENNGAY || !$scope.target.MAKHO_KHUYENMAI || $scope.target.DataDetails.length <= 0) {
                ...
                if ($scope.addItem.MANHOM && $scope.addItem.MANHOM.length > 0 && $scope.addItem.TENNHOM && $scope.addItem.TENNHOM.length > 0 && $scope.addItem.SOLUONG && $scope.addItem.SOLUONG != 0) {
                    Lobibox.notify('default', {
                        title: 'Nhắc nhở',
                        msg: 'Dòng loại hàng [' + $scope.addItem.MANHOM + '] chưa được thêm mới xuống danh sách, Bạn hãy thêm hoặc xóa trước khi lưu',
                        delay: 4000
                    });
                }
                else {
    ...
    app.controller('giamGiaNhomHangDetail_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'giamGiaNhomHangService', 'targetData', 'tempDataService', '$filter', '$uibModal', '$log',
        function ($scope, $uibModalInstance, $http, configService, service, targetData, tempDataService, $filter, $uibModal, $log) {
            ...
            $scope.title = function () { return 'Thông tin chương trình khuyến mãi giảm giá theo nhóm hàng [' + targetData.MA_KHUYENMAI + ']'; };
            $scope.convertCodeToName = function (paraValue, moduleName) {
                if (paraValue) {
                    var tempCache = $filter('filter')($scope.tempData(moduleName), { VALUE: paraValue }, true);
                    if (tempCache && tempCache.length === 1) {
    ...
    app.controller('giamGiaNhomHangEdit_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'giamGiaNhomHangService', 'targetData', 'tempDataService', '$filter', '$uibModal', '$log','userService','nhomHangService','khoHangService',
        function ($scope, $uibModalInstance, $http, configService, service, targetData, tempDataService, $filter, $uibModal, $log, userService, nhomHangService, khoHangService) {
            $scope.config = angular.copy(configService);
            $scope.paged = angular.copy(configService.pageDefault);
            $scope.tempData = tempDataService.tempData;
            var currentUser = userService.GetCurrentUser();
    ...
    app.controller('giamGiaNhomHangDelete_Ctrl', ['$scope', '$uibModalInstance', '$http', 'configService', 'giamGiaNhomHangService', 'targetData', 'tempDataService', '$filter', '$uibModal', '$log',
       function ($scope, $uibModalInstance, $http, configService, service, targetData, tempDataService, $filter, $uibModal, $log) {
           $scope.config = angular.copy(configService);
           $scope.tempData = tempDataService.tempData;
           $scope.target = {};
           $scope.target = angular.copy(targetData);;
           $scope.title = function () { return 'Xóa chương trình khuyến mãi giảm giá theo nhóm hàng [' + targetData.MA_KHUYENMAI + ']'; };
